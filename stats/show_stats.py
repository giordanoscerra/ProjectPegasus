...
def compare_heuristics(level:int=3):
    folder_path = 'stats/assessments'
    ...
    desired_sequence= str(level)+'Infinity'
    desired_level= 'L'+str(level)+'NANE'

    for file_path in [s for s in file_list if desired_sequence in s or desired_level in s]:
        # Specify the path to your text file
        pattern = r"\d+:\srewards:<([-\d.]+)>\ssteps:<([-\d.]+)>"
        rewards_list = []
        steps_list = []
    ...
    result_df = result_df.drop('Success Rate', axis=1)
    result_df = result_df.drop('Steps On Failure', axis=1)
    ...
    # Customize the plot (optional)
    plt.title('Infinity distance VS Manhattan distance')
    plt.xlabel('Level CODENAME')
    plt.ylabel('Steps')
    plt.legend(['Steps On Success'])
